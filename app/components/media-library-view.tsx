...
export function MediaLibraryView() {
  ...
  const folderDialogRef = useRef<HTMLElement>(null);
  const folderActivityRef = useRef<FolderActivity>('idle');
  const folderBrowseAbortRef = useRef<AbortController | undefined>(undefined);
  const folderBrowseRequestRef = useRef(0);

  const folderBusy = folderActivity !== 'idle';
  const folderCrumbs = folderBreadcrumbs(folderBrowser?.currentPath || '');
  ...
  useEffect(() => {
    ...
    const focusTimer = window.setTimeout(() => folderDialogRef.current?.focus(), 0);

    const handleDialogKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape') {
        event.preventDefault();
        if (folderActivityRef.current === 'loading') {
          folderBrowseRequestRef.current += 1;
          folderBrowseAbortRef.current?.abort();
          folderBrowseAbortRef.current = undefined;
          folderActivityRef.current = 'idle';
          setFolderActivity('idle');
        }
        setFolderOpen(false);
        return;
      }
      if (event.key !== 'Tab' || !folderDialogRef.current) return;

      const focusable = Array.from(folderDialogRef.current.querySelectorAll<HTMLElement>(
        'button:not([disabled]), input:not([disabled]), [href], [tabindex]:not([tabindex="-1"])',
      )).filter((element) => element.getAttribute('aria-hidden') !== 'true');
      if (focusable.length === 0) {
        event.preventDefault();
        folderDialogRef.current.focus();
        return;
      }
      const first = focusable[0];
      const last = focusable[focusable.length - 1];
      if (!folderDialogRef.current.contains(document.activeElement)) {
        event.preventDefault();
  ...
  useEffect(() => {
    const shortcut = (event: KeyboardEvent) => {
      if (folderDialogRef.current) return;
  ...
  const beginFolderActivity = (activity: Exclude<FolderActivity, 'idle'>) => {
    if (folderActivityRef.current !== 'idle') return false;
    folderDialogRef.current?.focus();
    folderActivityRef.current = activity;
    setFolderActivity(activity);
    return true;
  };

  const finishFolderActivity = () => {
    folderActivityRef.current = 'idle';
    setFolderActivity('idle');
  };

  const browseFolder = async (requestedPath: string) => {
    if (!beginFolderActivity('loading')) return;
    const requestId = folderBrowseRequestRef.current + 1;
    folderBrowseRequestRef.current = requestId;
    const controller = new AbortController();
    folderBrowseAbortRef.current = controller;
    let timedOut = false;
    ...
    try {
      const result = await jsonFetch<FolderBrowseResponse>(`/api/library/folders/browse?path=${encodeURIComponent(nextPath)}`, { signal: controller.signal });
      if (requestId !== folderBrowseRequestRef.current) return;
      ...
      if (requestId === folderBrowseRequestRef.current) {
        folderBrowseAbortRef.current = undefined;
  ...
  const openFolderBrowser = () => {
    if (folderActivityRef.current === 'scanning') {
      setFolderOpen(true);
      return;
    }
    setFolderError('');
  ...
  const addFolder = async () => {
    ...
    setFolderError('');
    try {
      await jsonFetch('/api/library/folders', { method: 'POST', body: JSON.stringify({ path: selectedPath }) });
      setFolderPath('');
      await loadLibrary();
      setFolderOpen(false);
    } catch (cause) {
      ...
      <section className="content" id="library">
        ...
        <div className="page-heading">
          ...
          {(server?.canPickLocalFolder || server?.canManageCloud) && <div className="library-actions">
            ...
            {server?.canPickLocalFolder && <button className="add-button" onClick={openFolderBrowser}><span className="folder-icon" aria-hidden="true" />添加媒体文件夹</button>}
          </div>}
        </div>
        ...
        {featured ? (
          ...
          <div className="empty-state"><span>＋</span><h2>添加第一个媒体文件夹</h2><p>电脑上的文件不会离开当前局域网。</p>{server?.canPickLocalFolder && <button className="add-button" onClick={openFolderBrowser}>选择文件夹</button>}</div>
        )}
